...
impl CursorController {
    ...
    pub(crate) fn move_cursor(&mut self, direction: char, editor_rows: &EditorRows, win_size: (usize, usize)) {
        ...
        let mut number_of_columns = editor_rows.get_row(self.cursor_y + self.row_offset).len();
        ...
        match direction {
            ...
            'd' => {
                ...
                else if self.column_offset != number_of_columns - 1 {
                    self.column_offset += 1;
                }
            }
            _ => unimplemented!(),
        ...
        number_of_columns = editor_rows.get_row(self.cursor_y + self.row_offset).len();
        if self.cursor_x + self.column_offset > number_of_columns {
            self.cursor_x = cmp::min(self.cursor_x, number_of_columns);
            self.column_offset = number_of_columns - self.cursor_x;
        }
    }
}
